...
public class VirtualMachine {
	...
	VirtualMachine(int cpuCount, long memoryGB, String guestOS){
		// Escribe tu código {
		if(!(cpuCount > 0)) {
			System.err.println("Alerta: CPU debe ser mayor a 0");
		...
		if(memoryGB < 1024) {
			System.err.println("Alerta: Memoria debe ser mayor o igual a 1024");
			this.memoryGB = 1024;
		}
		else {
		...
		if((this.memoryGB % 1024) != 0) {
			System.err.println("Alerta: Memoria debe ser multiplo de 1024");
			this.memoryGB = 1024;
		}
		else {
			this.memoryGB = memoryGB;
		}
		System.err.println("Alerta: Se recomienda nombres mayores a 5 caracteres");
		if(guestOS.length() < 5) {
			System.err.println("Alerta: Se recomienda nombres mayores a 5 caracteres");
		}
	...
	void powerOn() {
		...
		System.out.println("La maquina virtual " + this.id + " esta prendida.");
		// }
		}
	...
	void powerOff() {
		...
		System.out.println("La maquina virtual " + this.id + " esta apagada.");
		// }
		}
	...
	public String toString(){
		...
		sb.append(this.id);
		sb.append("\nNICs    : ");
		sb.append(this.nics.size());
		sb.append("\nHDDs    : ");
		sb.append(this.hdds.size());
		long capacidadHD = 0;
		for(int i = 1; i <= this.hdds.size(); i++) {
			capacidadHD += this.hdds.get(i).capacityGB;
		}
		sb.append("(");
		sb.append(capacidadHD);
		sb.append(" GB)\n");
